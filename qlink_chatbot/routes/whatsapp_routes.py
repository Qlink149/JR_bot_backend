...
def _currency_from_phone(phone: str) -> str:
    """Derive currency from E.164 phone number using calling code prefix."""
...
def _clean_for_whatsapp(text: str) -> str:
    ...
    text = re.sub(r'\n{3,}', '\n\n', text)
    # Strip leading numbering that the LLM sometimes adds (1. 2. 3.)
    text = re.sub(r'(?m)^\s*\d+\.\s+', '', text)
    return text.strip()


def _build_whatsapp_responses(text: str) -> list[dict]:
    """Split LLM Markdown into WhatsApp message dicts.

    Blocks with an image + View Product URL → interactive_cta (image header + button).
...
def _has_product_send(responses: list[dict]) -> bool:
    ...
    return any(
        isinstance(r, dict) and r.get("type") in product_types
        for r in responses
    )
...
async def _process_message(request_data: dict) -> None:
    ...
    try:
        ...
        # Derive currency from phone calling code — reliable without IP geo
        detected_currency = _currency_from_phone(phone_number)
